            ...
            <div id="main" class="main-content z-10 mt-12 md:mt-2 pb-24 md:pb-5">
                <div class="bg-gray-100 pt-3">
                    <div class="rounded-tl-3xl bg-gradient-to-r from-gray-100 to-sky-400 p-4 shadow text-2xl text-current">
                        <h1 class="font-bold pl-2">Grados</h1>
                    </div>
                </div>
                @if (session('success'))
            ...
            @endif
                <div class="flex flex-wrap w-full">
                    <div class="pt-10 py-10 px-10">
                        <button id="openModal" class="inline-flex items-center bg-sky-400 border-0 py-1 px-3 focus:outline-none hover:bg-gray-500 text-white rounded text-base mt-4 md:mt-0">Agregar grado</button>
                    </div>
                                        ...
                                        @foreach ($grado as $grados)
                                          <tr class="border-b border-neutral-200 dark:border-white/10">
                                              ...
                                              <td class="whitespace-nowrap px-6 py-4">
                                                <form id="deleteForm_{{ $grados->id }}" action="{{ route('grado.destroy', $grados->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button class="bg-red-400 text-white p-2 rounded" onclick="confirmarBorrado({{ $grados->id }})"><i class="fa fa-trash"></i></button>
                                                    <a href="{{ route('grado.edit', $grados->id) }}" class="bg-gray-400 text-white p-2 rounded"><i class="fa fa-pen"></i></a>
                                                </form>
                                              </td>
                                          </tr>
                                      @endforeach                                  
                                        ...
                                        <div class="modal-wrapper bg-white px-4 pt-5 pb-4 sm:p-6 sm:pd-4">
                                            <div class="modal-wrapper-flex sm:flex sm:item-start">
                                                ...
                                                <div class="modal-content text-center mt-3 sm:mt-0 sm:ml-4 sm:text-left">
                                                    <h3 class="text-lg font-medium text-gray-900">Agregar grados</h3>
